...
class Tutorial_Puzzles:    
    ...
    def LetsPlay(self, boton):
        ...
        nuevaVentana = Game_Window("TUTORIAL", int(boton))
        nuevaVentana.sonidos = self.sonidos
        self.window.Close()
        nuevaVentana.Update()
    ...
    def ShowTop(self, boton):
        ...
        nuevaVentana = Top_5("TUTORIAL", boton)
        nuevaVentana.sonidos = self.sonidos
        self.window.Close()
        nuevaVentana.Update()
    ...
    def PintarTops(self, p):
        for i in range(len(self.tops)):
            if p == i:
                self.window.Draw(self.tops2[i])
            else:
                self.window.Draw(self.tops[i])
